...
public static class TexFormatExtensions {
    ...
    public static Memory2D<ColorRgba32> SliceCopy(this Memory2D<ColorRgba32> pixels, uint sx, uint sy, uint height, uint width) {
        ColorRgba32[,] arr = pixels.ToArray();
        ColorRgba32[,] pixels2 = new ColorRgba32[height, width];
        for (uint x = sx; x < width + sx; x++) {
            for (uint y = sy; y < height + sy; y++) {
                pixels2[y - sy, x - sx] = arr[y, x];
            }
        }

        return new Memory2D<ColorRgba32>(pixels2);
    }

    public static Memory2D<ColorRgba32> CopyMerge(this Memory2D<ColorRgba32> pixels, Memory2D<ColorRgba32> pixels2, uint sx, uint sy, uint height, uint width) {
        ColorRgba32[,] target = pixels.ToArray();
        ColorRgba32[,] source = pixels2.ToArray();
        for (uint x = 0; x < width; x++) {
            for (uint y = 0; y < height; y++) {
                target[y + sy, x + sx] = source[y, x];
            }
        }

        return new Memory2D<ColorRgba32>(target);
    }
}